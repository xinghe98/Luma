...
连接时填写服务端 IP（例如 `192.168.1.10`）、端口和服务端生成的 API Token；客户端默认使用 HTTP。业务 API 默认使用 `/api/v1`，可通过 `--dart-define=LUMA_API_PREFIX=/api/v2` 覆盖。
...
- 管理员根 Token 会显示扫描、媒体源类型、待整理文件和“成员与访问管理”入口；成员 Token 根据 `/system/info` 的 capabilities 自动隐藏这些管理操作。
- “成员与访问管理”可创建或启停成员、授予媒体源访问权、签发和吊销设备 Token。新签发 Token 的明文只显示一次，客户端不会保存或再次展示它。
- 管理员仍可使用后端的 `scripts/issue-family-token.ps1` 或 `scripts/issue-family-token.sh` 完成批量或命令行管理。
